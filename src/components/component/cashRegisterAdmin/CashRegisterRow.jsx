...
// 					<ChevronDownIcon className={`h-4 w-4 ${isExpanded ? 'transform rotate-180' : ''}`} />
// 				</TableCell>
// 				<TableCell className="px-6 py-4 capitalize">{cashRegister.User.firstName} {cashRegister.User.lastName}</TableCell>
// 				<TableCell className="px-6 py-4">{cashRegister.date}</TableCell>
// 				<TableCell className="px-6 py-4">{cashRegister.CashBox.description}</TableCell>
// 				<TableCell className="px-6 py-4">$ {totalPaymentMethods}</TableCell>
// 				<TableCell className="px-6 py-4">$ {totalWithdrawalCards}</TableCell>
// 				<TableCell className="px-6 py-4">$ {totalWithdrawalCash}</TableCell>
// 				<TableCell className="px-3 py-4"><span className={`rounded-2xl p-1 text-xs ${cashRegister.isClosed ? 'bg-green-500' : 'bg-yellow-500'}`}>{cashRegister.isClosed ? 'Cerrada' : 'Abierta'}</span></TableCell> {/* TODO: agregar un badge */}
// 			</TableRow>
// 			{isExpanded && (
// 				<TableRow>
// 					<TableCell colSpan="8" className="px-6 py-4 whitespace-nowrap bg-gray-200/40">
...
// 									cashMovements={cashMovementsFiltered(cashRegister.id)}
// 									cancellations={cancellationsFiltered(cashRegister.id)}
// 									terminals={terminals.filter(terminal => terminal.cashRegisterId === cashRegister.id)}
// 								/>
// 								<div className='flex gap-2'>
// 									<CashMovementsTable cashMovements={cashMovementsFiltered(cashRegister.id)} />
// 									<CancellationsTable cancellations={cancellationsFiltered(cashRegister.id)} />
